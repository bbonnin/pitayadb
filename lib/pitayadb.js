...
class PitayaDB {
    ...
    static start(host, port, dataPath, logger) {
        new PitayaDB(host, port, dataPath, logger)
            .init()
            .then( () => logger.info("PitayaDB started, enjoy !") )
	    .catch( err => logger.error(err) )
    }

    static stop() {
        logger.info("PitayaDB stopped, see you soon !")
    }
}
